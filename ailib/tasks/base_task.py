...
from ailib.metrics import base_metric
from ailib.tools.utils_name_parse import parse_metric, parse_loss
...
class BaseTask(abc.ABC):
    ...
    def _assure_losses(self):
        if not self._losses:
            self._losses = self._convert(self.list_available_losses(), parse_loss)

    def _assure_metrics(self):
        if not self._metrics:
            self._metrics = self._convert(self.list_available_metrics(), parse_metric)
