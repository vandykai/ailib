...
class Matching(nn.Module):
    ...
    def forward(self, x, y):
        ...
        if self._matching_type == 'dot': # x,y shape [N, len, embed_size]
            if self._normalize:
                x = F.normalize(x, p=2, dim=-1)
                y = F.normalize(y, p=2, dim=-1)
            return torch.einsum('bld,brd->blr', x, y)
        elif self._matching_type == 'exact': # 当前元素的值在对方对应向量中出现的次数; x,y shape [N, len]
            ...
            return x, y
        else: # x,y shape [N, len, embed_size]
