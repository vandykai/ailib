...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if False and self.enable_torch_version and not self.onnx_trace and incremental_state is None and not static_kv:
        ...
        if self.bias_k is not None: #TODO:if self.bias_k is not None and not static_kv: #"bias cannot be added to static key value."
    ...
    def _append_prev_key_padding_mask(
        ...
        # is None
            ...
            if prev_key_padding_mask.is_cuda:
                filler = filler.cuda()
            #TODO:filler = torch.zeros(batch_size, src_len - prev_key_padding_mask.size(1), device=prev_key_padding_mask.device).bool()
            ...
            if key_padding_mask.is_cuda:
                filler = filler.cuda()
            #TODO:filler = torch.zeros(batch_size, src_len - key_padding_mask.size(1), device=key_padding_mask.device).bool()
    ...
    def _get_input_buffer(self, incremental_state):
        return utils_cache.get_incremental_state(
            self,
            incremental_state,
            'attn_state',
        ) or {}
    ...
    def _set_input_buffer(self, incremental_state, buffer):
        utils_cache.set_incremental_state(
            self,
            incremental_state,
            'attn_state',
            buffer,
