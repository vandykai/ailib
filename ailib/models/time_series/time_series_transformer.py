...
class Model(BaseModel):
    ...
    def __init__(self, config):
        ...
        self.bilstm = nn.GRU(bilstm_input_size, config.lstm_hidden_size)
    ...
    def forward(self, inputs):
        ...
        key_padding_mask = torch.ones(time_stamps.shape[:2], device=time_stamps.device).bool()
        ...
        # [seq_length, batch_size, embed_dim]
        question_feature_embeds = torch.cat([question_embeds,knowledge_embeds,logical_type_embeds,question_attention_embeds], dim=-1)
        sequence_output, _ = self.bilstm(question_feature_embeds[1:])
    ...
    def predict(self, inputs):
        with torch.no_grad():
            ...
            if 'current_question_embeds' in inputs:
                current_question_embeds = inputs["questions_embeds"].transpose(0, 1)
            else:
                current_question = inputs["current_questions"]
                current_question_embeds = torch.stack([self.question_embedding_model.encode(current_question, convert_to_tensor=True, show_progress_bar=False, device=time_stamps.device)], dim=1)

            question_knowledges = inputs["question_knowledges"]
            knowledge_embeds = self.knowledge_embed(question_knowledges).transpose(0, 1).mean(axis=-2)
            answer_embeds = self.answer_embed(question_answers).transpose(0, 1)
            # [seq_length, 1, embed_dim]
            question_feature_embeds = question_embeds + answer_embeds

            current_question_attention_embeds, attn_weights = self.multihead_attention(current_question_embeds, question_embeds, answer_embeds)
            current_question_attention_embeds += current_question_embeds
            # [seq_length, 1, embed_dim]
            all_question_feature_embeds = torch.cat([question_feature_embeds, current_question_attention_embeds])
            #_, (hn) = self.bilstm(all_question_feature_embeds[1:])
            #embs = self.dropout(embs)
            # [1, seq_length, embed_dim]
            print(all_question_feature_embeds[1:].shape)
            out = self.out(all_question_feature_embeds[1:]).transpose(0, 1)
            self.train()
            return out
