...
def get_parent_func(lineno, lines, ignore_missing=False):
    ...
    for idx,l in enumerate(reversed(lines[:lineno])):
        if re.match(r'\s*def test', l):  return (lineno - idx), l # 1 based index for github
        if re.match(r'\w+', l):  break # top level indent - out of function scope
